// ── Zerodha Instrument Master Sync Service ──
...
export class InstrumentsService {
  ...
  syncFromRaw(rawRows: RawInstrumentCsvRow[]): InstrumentSyncResult {
    ...
    // --- Phase 2: Persist ---
    // Use a transaction to ensure atomicity — instruments + sync state
    // are written together. On error, neither persists.
    let insertedCount = 0;
    try {
      ...
      // Record the failure event but do NOT update sync state —
      // the last successful snapshot is preserved.
      this._repo.insertIngestionEvent({
        ...
        itemCount: null,
        error: errorMsg,
        diagnostic: { totalRows: rawRows.length, skipped },
      });
